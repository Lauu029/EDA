...
public:
	...
	void cuela(const T& a, const T& b) {
		...
		while (victima->elem != a && victima != nullptr) victima = victima->sig;
		if (victima != nullptr) {
			colar = victima->sig;
			sigV = victima->sig;
			while (colar->elem != b && colar != nullptr) colar = colar->sig;
			if (colar != nullptr) {
				victima->sig->sig = colar->sig;
				colar->sig = victima->sig;
				victima->sig = colar;
			}
		}
	}
