...
protected:
	...
	void hojas(Link a, int& hoj) {
		if (a != nullptr) {
			if (a->left == nullptr && a->right == nullptr)hoj++;
			else {
				hojas(a->left, hoj);
				hojas(a->right, hoj);
			}
	...
	void altura(Link a, int alt,int& max) {
		if (a != nullptr) {
		
			altura(a->left, alt + 1, max);
			altura(a->right, alt + 1, max);
		}
		else {
			if (max < alt)max = alt;
			alt--;
