...
public:
	...
	void multiplo(int& mult, int& prof) {

		encuentraMult(raiz, mult, 0, prof);
...
protected:
	...
	void encuentraMult(Link a, int& mult, int prof, int& profMult) {
		if (a != nullptr) {
			if (!compruebaPrimo(a->elem)) {
				if (a->elem % 7 == 0) {
					if (profMult > prof) {
						mult = a->elem;
						profMult = prof;
					...
					encuentraMult(a->left, mult, prof + 1, profMult);
					encuentraMult(a->right, mult, prof + 1, profMult);
				}
			}
		}
	}
