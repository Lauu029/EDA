// Repaso examenes EDA.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
using namespace std;
/*
* Vamos a mirar varios ejercicios para el examen
* empezamos con backtracking
*	ejercicio del concierto(es un evaluable)
*/
...
/*//Método que devuelve si un artista puede actuar después de otro
    ...
    void resolver(vector<vector<int>> beneficios, const vector<int> mejoresBeneficiosDesde; vector<vector<bool>> consentimientos, vector<int>& actuado, vector<bool>& yaExiste, int& maxBeneficios, int& ganado, int k, int n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (k == 0 || actua(consentimientos, i, actuado[k - 1], yaExiste))
            {
                ...
                //Poda
                if(ingresos+mejoresBeneficiosDesde[k+1]>mejorSolucion){
    ...
    void resuelveCaso() {
        ...
        for(jin i=0;i<n;i++){
            for(...){
                if(...>maxBenefs[i])maxBenefs[i]=...;
            }
        }
        vector<int> mejoresBeneficiosDesde(n);
        mejoresBeneficiosDesde[n-1]=maxBenefs[n-1];
        for(int i=n-2;i<n;i++){
            mejoresBeneficiosDesde[i]+=mejoresBeneficiosDesde [i-1];(creo)
        int maxBeneficios = 0;
        int ganado = 0;
