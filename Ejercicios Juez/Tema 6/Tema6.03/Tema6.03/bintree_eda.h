...
protected:
   ...
   bintree(Link const& r) : raiz(r) {}
    ...
    //Método que busca el elemento menor de un arbol
    void encuentraMenor(Link a, T& min, T& aux) {
        if (a != nullptr) {
            encuentraMenor(a->left, min, aux);
            encuentraMenor(a->right, min, aux);
            aux = a->elem;
            if (min > aux) min = aux;
        }
    }
public:
   ...
   //Método público que llama al buscador del elemento más pequeño
   T menor() {
       T s = raiz->elem;
       T aux = raiz->elem;
       encuentraMenor(raiz, s, aux);
       return s;
   }
