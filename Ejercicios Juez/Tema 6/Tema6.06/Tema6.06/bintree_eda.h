...
public:
	...
	void childLeft(bintree<T>arbol) {
		raiz->left = std::make_shared<TreeNode>(nullptr, arbol, nullptr);
	}
	void childRight(bintree<T>arbol) {
		raiz->right = std::make_shared<TreeNode>(nullptr, arbol, nullptr);
	}
