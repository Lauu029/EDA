...
void resolver(const vector<vector<int>>& beneficios, const vector<vector<bool>>& consentimientos, vector<int>& actuado,
	vector<bool>& yaExiste, int& maxBeneficios, int& ganado, int k, const int& n, const vector<int>optimo)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (k == 0 || actua(consentimientos, i, actuado[k - 1], yaExiste))
		{
			...
			else if (ganado + optimo[k + 1] > maxBeneficios) {

				yaExiste[i] = true;
				resolver(beneficios, consentimientos, actuado, yaExiste, maxBeneficios, ganado, k + 1, n, optimo);
				yaExiste[i] = false;
			}
		}
		ganado -= h;
...
void resuelveCaso() {
	...
	for (int i = n - 1; i >= 0; i--) {
		for (int j = 0; j < n; j++) {
			if (beneficios[j][i] > max)max = beneficios[j][i];
		}
		suma += max;
		optimo[i] = suma;
	}

	resolver(beneficios, consentimientos, actuado, yaExiste, maxBeneficios, ganado, 0, n, optimo);
	// salida
