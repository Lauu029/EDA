...
public:
    ...
    {
        //nlog(n)
        /*si hay menos elementos en el segundo array que en el primero,
        el primero no puede ser subconjunto, devuelve false*/
        /*if (set.nelems < nelems)
            return false;
        else if (nelems == 0)
            return true;
        ...
        while (i < set.nelems && !encontrado)
        {
            if (set.array[i] == array[j]) 
                j++;
        ...
        //si el primer vector ha llegado al final del segundo 
        if (j == nelems)
            return true;
        return false;
    }
